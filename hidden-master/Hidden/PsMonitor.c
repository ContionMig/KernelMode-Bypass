...
OB_PREOP_CALLBACK_STATUS ProcessPreCallback(PVOID RegistrationContext, POB_PRE_OPERATION_INFORMATION OperationInformation)
{
	...
	PsLookupProcessByProcessId(ProtectedProgramPID, &ProtectedProcessProcess); // Getting the PEPROCESS using the PID 
	PsLookupProcessByProcessId(LsassPID, &LsassProcess); // Getting the PEPROCESS using the PID 
	PsLookupProcessByProcessId(CsrssPID, &Csrss1Process); // Getting the PEPROCESS using the PID 
	PsLookupProcessByProcessId(CsrssSecondPID, &Csrss2Process); // Getting the PEPROCESS using the PID 
	...
	if (OpenedProcess == ProtectedProgramPID) // Making sure that the game can open a process handle to itself
		return OB_PREOP_SUCCESS;
	...
	if (PsGetProcessId((PEPROCESS)OperationInformation->Object) == ProtectedProgramPID)
	{
